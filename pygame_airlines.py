...
from pygame_common import UIColors, UIConstants
...
class AirlinesWindow:
    ...
    def __init__(self, visualizer: PygameAirlinesVisualizer) -> None:
        ...
        self.drone_sprite = self._load_outlined_sprite("drone.png", (26, 26))
        self.car_sprite = self._load_outlined_sprite("car.png", (25, 25))
        ...
        for event in self.visualizer.event_queue:
            if hasattr(event, "agent_label"):
                self._get_flight_info(getattr(event, "agent_label", ""))
    ...
    def _load_outlined_sprite(
        self, filename: str, size: tuple[int, int]
    ) -> Optional[pygame.Surface]:
        """Loads a sprite and applies a white outline using masks."""
        try:
    ...
    def _draw_map(self) -> None:
        for connection in self.graph.connections:
            ...
            condition = self.connection_weather.get(
        ...
        for zone in self.graph.zones.values():
            ...
            pygame.draw.circle(
                self.screen, node_color, (zone.x, zone.y), 10
            )
            pygame.draw.circle(
        ...
        for (point, is_car), labels in drone_groups.items():
            ...
            if len(labels) > 1:
                count_surf = self.badge_font.render(
                    str(len(labels)), True, UIColors.WHITE
                )
                badge_r = max(8, count_surf.get_width() // 2 + 3)
                badge_pos = (point[0] + 12, point[1] - 12)

                pygame.draw.circle(
                    self.screen, UIColors.RED, badge_pos, badge_r
                )
                pygame.draw.circle(
                    self.screen, UIColors.WHITE, badge_pos, badge_r, 1
                )
    ...
    def _draw_sidebar(self) -> None:
        ...
        y_offset = self._draw_sidebar_header(sb_x)
        if self.connection_weather:
            y_offset = self._draw_sidebar_weather_alerts(sb_x, y_offset)
        self._draw_sidebar_live_departures(sb_x, y_offset)

    def _draw_sidebar_header(self, sb_x: int) -> int:
        y = 25
    ...
    def _draw_sidebar_weather_alerts(self, sb_x: int, y: int) -> int:
    ...
    def _draw_sidebar_live_departures(self, sb_x: int, y: int) -> None:
        f_title = self.city_font.render(
            "LIVE DEPARTURES", True, UIColors.CYAN
        )
        ...
        for drone_label, info in sorted(self.flight_data.items()):
            ...
            if y > self.height - 60:
                more_surf = self.dashboard_font.render(
                    "... tracking more flights", True, UIColors.TEXT_MUTED
                )
    ...
    def _determine_flight_status(
        ...
        if zone_a and zone_b:
            conn = self.graph.get_connection(zone_a, zone_b)
            if conn:
                weather = self.connection_weather.get(conn.name(), "clear")
                is_road = conn.distance < 200

                if weather in ("storm", "snow"):
                    return (
                        "ROAD DELAY" if is_road else "DELAYED",
                        UIColors.RED,
                    )
                return (
                    "DRIVING" if is_road else "EN ROUTE",
                    UIColors.BLUE if is_road else UIColors.GREEN,
                )
